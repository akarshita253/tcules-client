    ...
    <Section>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 bg-neutral-50 rounded-xl p-2">
        ...
        {thirdSectionData?.cards &&
          ...
          thirdSectionData?.cards?.map((card) => {
            return (
              <div
                className="relative p-8 flex flex-col bg-noise justify-between gap-4 overflow-hidden bg-neutral-100 rounded-xl"
                key={card?.id}
              >
                <div className="flex justify-end">
                  <ButtonTag href={card?.href || "#"}>
                    {card?.icon && (
                      <Image
                        src={card?.icon?.url}
                ...
                <div>
                  <h3 className="text-heading-xs text-neutral-900 mb-4 sm:mb-6">
                    {card?.heading}
                  </h3>
