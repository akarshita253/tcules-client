...
const SinglePodcastContainer = ({
  ...
  singlePodcastData: NonNullable<PodcastsQuery["podcasts"]>[0];
}) => {
  if (!singlePodcastData) return null;
  const { title, featureImage, tags, description } = singlePodcastData;
  return (
    <>
          ...
          <Section>
            <div className="mb-6 sm:mb-12">
              ...
              <div className="flex gap-2 items-center text-label-xs text-neutral-600">
                <span>{formatDate(singlePodcastData?.createdAt)}</span>
                <span>.</span>
              </div>
...
export default SinglePodcastContainer;
