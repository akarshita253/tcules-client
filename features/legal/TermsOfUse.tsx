...
type TermsOfUseData = TermsOfUseQuery["termsOfUse"];

interface TermsOfUseProps {
  termsOfUseData: TermsOfUseData;
}

const TermsOfUseContainer = ({ termsOfUseData }: TermsOfUseProps) => {
  if (!termsOfUseData) return null;
  return (
    <Container className="md:grid grid-cols-12 gap-6">
      <Block className="lg:col-start-2 col-start-1 lg:col-end-12 col-end-13">
        <Section>
          <div className="flex flex-col gap-4">
            <h1 className="text-heading-sm text-neutral-800">
              {termsOfUseData?.heading}
            </h1>
            <small className="text-label-sm text-neutral-500">
              Last updated on {formatDate(termsOfUseData?.updatedAt)}
            </small>
        ...
        <Section>
          {termsOfUseData?.legalDescription !== undefined &&
            termsOfUseData?.legalDescription &&
            termsOfUseData?.legalDescription?.length > 0 &&
            termsOfUseData.legalDescription.map((content, index) => {
